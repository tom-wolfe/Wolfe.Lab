...
# `beszel/deploy` by ./flow.yaml beside it, which chains on every green
...
#
# The AGENT is deliberately not touched here. It's a Homebrew formula with a
# `brew services` block, declared in the Brewfile, and converged by
# `lab.chezmoi/packages` (install) and `lab.chezmoi/packages-upgrade`
# (versions). One converger per thing, and host packages already have one.
set -euo pipefail
