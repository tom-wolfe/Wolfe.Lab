...
internal sealed class ImportTakeout(IDocker docker, ISecrets secrets, ImmichServer server, ImportOptions options, IWorkflowLog log)
{
    ...
    {
        var key = await secrets.Read(server.ApiKey, ct);
