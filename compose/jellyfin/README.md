...
```sh
cd ~/Development/Wolfe.Tools/jellyfin
...
```sh
open -a Docker && cd ~/Development/Wolfe.Tools/jellyfin && docker compose up -d
```
...
```sh
./backup.sh                  # config + database + library roots + plugins
./backup.sh --with-metadata  # the above plus ~670 MB of posters/fanart
```

Writes a timestamped tarball to `~/Docker/jellyfin/backups/` and keeps the last
10. It stops the container first — SQLite copied mid-write can be inconsistent —
so expect ~30s of downtime.
...
tar -xzf ~/Docker/jellyfin/backups/jellyfin-YYYYmmdd-HHMMSS.tar.gz \
...
```sh
cd ~/Development/Wolfe.Tools/jellyfin && docker compose down
mv ~/Library/Application\ Support/jellyfin ~/Library/Application\ Support/jellyfin.bak
